...
def get_user_inputs(index_names):
    ...
    index_choice = st.sidebar.selectbox("Choose Index Benchmark", index_names)
    start_age = st.sidebar.number_input("Enter Starting Age", min_value=40, max_value=80, value=55, step=1)
    premium = st.sidebar.number_input("Enter Starting Balance", min_value=0.0, value=1000000.0, step=10000.0)
    pr_start = st.sidebar.number_input("Enter Starting FIA PR", 0.0, 1.0, 1.0, 0.01)
    pr_end = st.sidebar.number_input("Enter Ending FIA PR", 0.0, 1.0, 0.35, 0.01)
    floor = st.sidebar.number_input("Enter FIA Floor Rate", 0.0, 0.10, 0.0, 0.01)
    fee = st.sidebar.number_input("Enter 401(k) Annual Fee Rate", 0.0, 0.05, 0.02, 0.005)
    inflation = st.sidebar.number_input("Enter Annual Inflation Rate", 0.0, 0.1, 0.03, 0.005)
    tax = st.sidebar.number_input("Enter Tax Rate on RMDs", 0.0, 0.5, 0.30, 0.01)
